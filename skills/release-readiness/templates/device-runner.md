...
- {{.ID}}: {{.Name}} ({{.Platform}}, {{.Framework}}){{if .Width}}, cards {{.Width}} px wide{{end}}
  ...
  - run: `{{.Run}}`
{{- end}}
- At most {{.C.Devices.ConcurrentDevices}} devices at once (claude-guards simCap here: {{.SimCap}}). {{with div .C.Devices.ConcurrentDevices (len .MacDevices)}}That is {{.}} full set(s) of the matrix, so serve that many lanes concurrently and never more.{{else}}That is less than one full set: keep at most {{.C.Devices.ConcurrentDevices}} devices booted, {{with .C.Devices.Realtime}}boot a realtime run's {{len .Roles}} role devices together, {{end}}bring up single-device platforms in turn within that budget, shut down what the next run does not need, and serve one lane at a time.{{end}}
