...
| `vybava setup team` | verb | One-click henderson-tech Mac — the `henderson` group's tools (Onyx + Helium, vitrinka, SwitcherooBar, Pultík; devbox opt-in) through their own channels plus the git family, as a checklist or `--yes --json`. → [docs/setup-team.md](docs/setup-team.md) |
