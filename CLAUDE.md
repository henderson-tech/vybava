...
`internal/plugingc` garbage-collects the Claude Code plugin cache. Two rules are
load-bearing and documented in `docs/plugin-gc.md`: the active version comes
from `installed_plugins.json` compared BY PATH — never by sorting version
strings, which puts `3.11.0` above `5.3.0` — and a `.in_use` marker is dead
only when proven so (PID gone, PID held by something that cannot be a session,
or a process that started AFTER the marker's own mtime). `kill(pid, 0)`
succeeding proves nothing; PIDs are recycled. Everything undecidable is held,
and destruction only ever happens behind `--apply`.
