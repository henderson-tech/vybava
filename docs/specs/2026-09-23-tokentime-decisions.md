...
| 11 | Project detail (2026-09-24) | A separate read-only `project` verb — one root, inclusive local days, a zero-filled hour/day/month series — rather than widening `rollup`; no index pass, no lock | The rollup is sized for the always-on bar (every project, fixed windows); an expanded project row needs any range and a dense series for one project, on demand, and must answer at once even mid-pass. Folding, names, day cutting and the run rule are the rollup's own code (`projects`, `windowRows`, `eachRun`), so the two views cannot disagree. Default bucket: hour for one day, month past 62 days, else day |
